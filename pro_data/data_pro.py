...
MAX_VOCAB = 50000
DOC_LEN = 1000
# the pre-trained word2vec files
...
if __name__ == '__main__':
    ...
    data.to_csv(f'{save_folder}/data.csv', index=None)
    ...
    with open(f'{save_folder}/user2index', 'wb') as handle:
        pickle.dump(user2id, handle, protocol=pickle.HIGHEST_PROTOCOL)

    with open(f'{save_folder}/index2user', 'wb') as handle:
        pickle.dump(id2user, handle, protocol=pickle.HIGHEST_PROTOCOL)

    with open(f'{save_folder}/item2index', 'wb') as handle:
        pickle.dump(item2id, handle, protocol=pickle.HIGHEST_PROTOCOL)

    with open(f'{save_folder}/index2item', 'wb') as handle:
        pickle.dump(id2item, handle, protocol=pickle.HIGHEST_PROTOCOL)
    ...
    data_train.to_csv(f'{save_folder}/data_train.csv', index=None)
    data_val.to_csv(f'{save_folder}/data_val.csv', index=None)
    data_test.to_csv(f'{save_folder}/data_test.csv', index=None)



    uidList_train, iidList_train = get_count(data_train, 'user_id'), get_count(data_train, 'item_id')

    uidList_test, iidList_test = get_count(data_test, 'user_id'), get_count(data_test, 'item_id')
    uidList_val, iidList_val = get_count(data_val, 'user_id'), get_count(data_val, 'item_id')


    userNum = len(uidList_train)
    itemNum = len(iidList_train)

    userNum_test = len(uidList_test)
    itemNum_test = len(iidList_test)

    userNum_val = len(uidList_val)
    itemNum_val = len(iidList_val)
    ...
    # save word2idx idx2word vocab
    with open(f'{save_folder}/word2index', 'wb') as handle:
        pickle.dump(word_index, handle, protocol=pickle.HIGHEST_PROTOCOL)

    with open(f'{save_folder}/index2word', 'wb') as handle:
        pickle.dump(index_word, handle, protocol=pickle.HIGHEST_PROTOCOL)

    with open(f'{save_folder}/vocab', 'wb') as handle:
    ...
    if PRE_W2V_BIN_PATH:
        ...
        pre_word2v = gensim.models.KeyedVectors.load_word2vec_format(PRE_W2V_BIN_PATH, binary=binary)
    else:
        pre_word2v = {}
